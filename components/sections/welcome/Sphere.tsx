...
export default function Sphere() {
	...
	useEffect(() => {
		...
		const handleMouseMove = (event: MouseEvent) => {
			...
			mouseWorld.copy(raycaster.ray.origin).add(raycaster.ray.direction.multiplyScalar(distance))
		}
		...
		const animate = () => {
			...
			if (isIntro) {
				...
				for (let i = 0; i < positions.count; i++) {
					...
					positions.setXYZ(i, baseX * tPoint, baseY * tPoint, baseZ * tPoint)
				}
	...
	useEffect(() => {
		...
		if (lineMaterialRef.current) {
			...
			lineMaterialRef.current.blending = isDark ? THREE.AdditiveBlending : THREE.NormalBlending
